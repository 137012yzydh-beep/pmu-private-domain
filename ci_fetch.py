...
.floating-red::after { content:'🔴'; font-size:14px; position:absolute; inset:0; display:flex; align-items:center; justify-content:center; }
...
.data-io-row { display:flex; gap:8px; margin-bottom:12px; flex-wrap:wrap; }
.data-io-row button { background:#333; color:#aaa; border:1px solid #444; padding:4px 12px; border-radius:4px; font-size:12px; cursor:pointer; }
.data-io-row button:hover { background:#444; color:#fff; }
#importSleepFile, #importDietFile { display:none; }
</style>
</head>
<body>
  ...
  <div class="modal slp-modal">
    ...
    <div class="edit-form" id="sleepEditForm">
      ...
      <div class="data-io-row">
        <button onclick="exportSleepData()" title="导出睡眠数据为JSON文件">⬇ 导出</button>
        <button onclick="document.getElementById('importSleepFile').click()" title="从JSON文件导入睡眠数据">⬆ 导入</button>
        <input type="file" id="importSleepFile" accept=".json" onchange="importSleepData(event)">
      </div>
      ...
      <div id="sleepAddForm" style="display:none;">
        ...
        <div style="font-size:12px;color:#888;margin-bottom:8px;">💤 睡眠阶段（分钟）</div>
        <div class="form-row">
...
<div class="modal-overlay" id="dietModal">
  <div class="modal diet-modal">
    ...
    <div class="edit-form" id="dietEditForm">
      ...
      <div class="data-io-row">
        <button onclick="exportDietData()" title="导出饮食数据为JSON文件">⬇ 导出</button>
        <button onclick="document.getElementById('importDietFile').click()" title="从JSON文件导入饮食数据">⬆ 导入</button>
        <input type="file" id="importDietFile" accept=".json" onchange="importDietData(event)">
      </div>
...
function renderTriplePanels(days) {
  ...
  days.forEach(d => {
    ...
    if (data && (data.cardio?.length || data.strength?.length)) {
      ...
      if (data.cardio) data.cardio.forEach(c => {
        trainHTML += '🚴 '+c.type+' \u00B7 '+secToHMS(c.time)+' \u00B7 '+c.kcal+'kcal<br>';
      });
      if (data.strength) data.strength.forEach(s => {
        trainHTML += '💪 '+s.name+' \u00B7 '+secToHMS(s.time)+' \u00B7 '+s.kcal+'kcal<br>';
      });
      trainHTML += '</div>';
    } else {
...
function exportSleepData() {
  ...
  a.download = 'pmu_sleep_backup_' + new Date().toISOString().slice(0,10) + '.json';
  a.click();
  URL.revokeObjectURL(a.href);
}
function importSleepData(e) {
  ...
  reader.onload = function(ev) {
    try {
      ...
      const existing = JSON.parse(localStorage.getItem(STORAGE_KEY_SLEEP) || '{}');
      const merged = Object.assign({}, existing, imported);
      localStorage.setItem(STORAGE_KEY_SLEEP, JSON.stringify(merged));
      SLEEP_DATA = merged;
      alert('导入成功！合并了 ' + Object.keys(imported).length + ' 天的睡眠记录。');
      if (sleepAuthed) { showSleepEditor(); }
    } catch(err) {
      alert('导入失败：文件格式不正确。');
    }
  };
...
function exportDietData() {
  const data = localStorage.getItem(STORAGE_KEY_DIET) || '{}';
  const blob = new Blob([data], {type:'application/json'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'pmu_diet_backup_' + new Date().toISOString().slice(0,10) + '.json';
  a.click();
  URL.revokeObjectURL(a.href);
}
function importDietData(e) {
  const file = e.target.files[0];
  if (!file) return;
  const reader = new FileReader();
  reader.onload = function(ev) {
    try {
      const imported = JSON.parse(ev.target.result);
      if (typeof imported !== 'object' || Array.isArray(imported)) throw new Error('格式错误');
      const existing = JSON.parse(localStorage.getItem(STORAGE_KEY_DIET) || '{}');
      const merged = Object.assign({}, existing, imported);
      localStorage.setItem(STORAGE_KEY_DIET, JSON.stringify(merged));
      DIET_DATA = merged;
      alert('导入成功！合并了 ' + Object.keys(imported).length + ' 天的饮食记录。');
      if (dietAuthed) { showDietEditor(); }
    } catch(err) {
      alert('导入失败：文件格式不正确。');
    }
  };
